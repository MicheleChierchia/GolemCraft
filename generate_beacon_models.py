import os
import json

states = [
    ("golem_beacon", "copper_block"),
    ("golem_beacon_exposed", "exposed_copper"),
    ("golem_beacon_weathered", "weathered_copper"),
    ("golem_beacon_oxidized", "oxidized_copper")
]

base_dir = "/home/miky/IdeaProjects/GolemCraft/src/main/resources/assets/golemcraft/models/block/"
os.makedirs(base_dir, exist_ok=True)

for model_name, copper_tex in states:
    model_json = {
        "parent": "minecraft:block/block",
        "render_type": "minecraft:cutout",
        "textures": {
            "particle": "minecraft:block/glass",
            "glass": "minecraft:block/glass",
            "obsidian": f"minecraft:block/{copper_tex}",
            "beacon": "golemcraft:block/golem_beacon_core"
        },
        "elements": [
            {
                "__comment": "Glass shell",
                "from": [ 0, 0, 0 ], "to": [ 16, 16, 16 ],
                "faces": {
                    "down":  { "uv": [ 0, 0, 16, 16 ], "texture": "#glass" },
                    "up":    { "uv": [ 0, 0, 16, 16 ], "texture": "#glass" },
                    "north": { "uv": [ 0, 0, 16, 16 ], "texture": "#glass" },
                    "south": { "uv": [ 0, 0, 16, 16 ], "texture": "#glass" },
                    "west":  { "uv": [ 0, 0, 16, 16 ], "texture": "#glass" },
                    "east":  { "uv": [ 0, 0, 16, 16 ], "texture": "#glass" }
                }
            },
            {
                "__comment": "Copper base",
                "from": [ 2, 0.1, 2 ], "to": [ 14, 3, 14 ],
                "faces": {
                    "down":  { "uv": [ 2,  2, 14, 14 ], "texture": "#obsidian" },
                    "up":    { "uv": [ 2,  2, 14, 14 ], "texture": "#obsidian" },
                    "north": { "uv": [ 2, 13, 14, 16 ], "texture": "#obsidian" },
                    "south": { "uv": [ 2, 13, 14, 16 ], "texture": "#obsidian" },
                    "west":  { "uv": [ 2, 13, 14, 16 ], "texture": "#obsidian" },
                    "east":  { "uv": [ 2, 13, 14, 16 ], "texture": "#obsidian" }
                }
            },
            {
                "__comment": "Inner beacon core",
                "from": [ 3, 3, 3 ], "to": [ 13, 14, 13 ],
                "faces": {
                    "down":  { "uv": [ 3, 3, 13, 13 ], "texture": "#beacon" },
                    "up":    { "uv": [ 3, 3, 13, 13 ], "texture": "#beacon" },
                    "north": { "uv": [ 3, 2, 13, 13 ], "texture": "#beacon" },
                    "south": { "uv": [ 3, 2, 13, 13 ], "texture": "#beacon" },
                    "west":  { "uv": [ 3, 2, 13, 13 ], "texture": "#beacon" },
                    "east":  { "uv": [ 3, 2, 13, 13 ], "texture": "#beacon" }
                }
            }
        ]
    }
    with open(os.path.join(base_dir, f"{model_name}.json"), "w") as f:
        json.dump(model_json, f, indent=2)

print("Generated 3D Block Models.")
